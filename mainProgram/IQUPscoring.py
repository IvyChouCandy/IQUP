...
class Scoring:
    def __init__(self, predVectorList, probVectorList, answerDf, modelNameList=None):
        ...
        if modelNameList is None:
            modelNameList = ['gbc', 'ridge', 'lr', 'catboost', 'lda', 'ada', 'knn', 'nb', 'et', 'lightgbm', 'rf', 'xgboost', 'mlp', 'dt', 'qda']#svm gpc rbfsvm拿掉
    ...
    def optimizeMcc(self, cutOffList=None, method='mcc', bestCutoffJsonPath=None):
        ...
        for (modelName, mccMax) in zip(self.modelNameList, mccMaxList):
            ...
            mccScoreDict[modelName] = scoreDf[modelName].loc[mccMax]
            bestPredArrList.append(bestPredArr)
            bestPredArrDict[modelName] = bestPredArr
            bestProArrDict[modelName] = probDf[modelName]
        ...
        maxModelName = max(mccScoreDict,key=lambda  x: mccScoreDict[x])
        bestCutoff = mccMaxDict[maxModelName]
        bestPredVector = predArrDf[maxModelName].loc[bestCutoff]
        bestProVector = probDf[maxModelName]

        return bestProArrDict,bestPredArrDict,bestPredArrList,bestPredVector,bestProVector
